...
tactile_method <- function() {
  xyplot(fit, which = 1:2)
}

f <- microbenchmark::microbenchmark(stats_method(), tactile_method())
```

And next we plot the results. As you see, the tactile method is slower
...
This function is similar to `xyplot.lm()` but is modelled after
...
standardized residuals as well, but also that we're correcting the 
the Ljung--Box test to account for the fact that we've fit a model
([see this](http://www.stat.pitt.edu/stoffer/tsa4/Rissues.htm)).
