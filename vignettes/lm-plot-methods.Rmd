...
trelliswork provides an `xyplot()` method for `lm` objects -- usually generated
...
Or instead use the method provided by trelliswork.
...
`xyplot.lm()` from trelliswork are that
...
trelliswork_method <- function() {
  xyplot(fit, which = 1:2)
}

f <- microbenchmark::microbenchmark(stats_method(), trelliswork_method())
```

And next we plot the results. As you see, the trelliswork method is slower
